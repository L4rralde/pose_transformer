...
from poseformer.models.eupe_pose_former import Former
...
def main():
    ...
    eupe = torch.hub.load(
        ...
        'eupe_vits16',
        source='local',
        weights='eupe_weights/EUPE-ViT-S.pt'
    )
    model = Former(eupe, 4).to(device)
    optim = torch.optim.AdamW(model.get_trainable_parameters())
